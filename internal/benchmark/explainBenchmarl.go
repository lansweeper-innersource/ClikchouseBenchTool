...
func ExplainBenchmark(ctx context.Context, conn clickhouse.Conn, query string) (internal.QueryExplainBenchmarkResult, error) {
	result := internal.QueryExplainBenchmarkResult{}
	explainQuery := fmt.Sprintf("EXPLAIN indexes = 1, json = 1, description = 1 %s FORMAT TSVRaw", query)

	explainRow, err := conn.Query(ctx, explainQuery)
	if err != nil {
		return result, fmt.Errorf("run explain benchmark query: %w", err)
	}
	...
	if err != nil {
		return result, fmt.Errorf("scan explain query rows: %w", err)
	}
	...
	if err != nil {
		return result, fmt.Errorf("unmarshal explain result: %w", err)
	}
	...
	if err != nil {
		return result, fmt.Errorf("build jsonpath query to analyze explain query results: %w", err)
	}
	data, err := path(context.Background(), resultsData)
	if err != nil {
		return result, fmt.Errorf("execute jsonpath query to analyze explain query results: %w", err)
	}
	...
	for _, elem := range flatData {
		...
		resultsMap := internal.QueryExplainResultData{
